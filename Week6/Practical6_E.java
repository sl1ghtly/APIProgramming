...
public class Practical6_E extends Application
{
   ...
   public void start(Stage primaryStage)
   {
     ...
     btCalc.setOnAction(listener); 
   }
   
   // 1. Listener inner class that implements the EventHandler interface 
   class Listener implements EventHandler<ActionEvent> {
      @Override
      // 2. Override the handle() method from the EventHandler interface
      public void handle(ActionEvent e) { 
         // getText() in textAmount
         try {
            double amount = Double.valueOf(textAmount.getText());
            int years = Integer.valueOf(textYears.getText());
            double rate = Double.valueOf(textRate.getText());
            
            double futureValue = amount + (amount * (rate / 100) * years);
            textFuture.setText("" + futureValue);
         }
